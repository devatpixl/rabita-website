...
// The social channels, and the brand marks that stand for them.
//
// Extracted from follow-us.tsx on 2026-09-16, when the footer was asked for
// "logo til alle sosiale medier nederst" (client). Before that the site held
// the same four channels in two places — the cards on the home page and a
// row of plain text links in the footer — with three of the four URLs typed
// out twice. A channel that moves, or a fifth one that joins, now changes in
// exactly one file.
//
// No 'use client'. Nothing in here is stateful, so the same component renders
// in follow-us.tsx (a server component) and in footer.tsx (a client one).
...
// in the card.
export const CHANNEL_RGB: Record<ChannelKey, string> = {
